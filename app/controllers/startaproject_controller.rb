class StartaprojectController < ApplicationController
  ...
  def deleteinvest
	...
	def updateremit
		...
		if @set.save
			if @set.bank.nil? or @set.time.nil? or @set.account_name.nil? or @set.account_num.nil? or @set.bank==0 or @set.time==0 or @set.account_name==0 or @set.account_num==0
				flash.now[:alert] = "請確實填寫每一個欄位"
    		redirect_to :action => :remit, :id => params[:id]
			else
				flash.now[:alert] = "謝謝您的贊助，我們將會盡快審核款項"
  ...
  def myindex	#我的專案，列出自己提出的專案
    ...
    @m_userproon = Member.includes(:projectparam).includes(:presub).includes(:pro).where("(projectparams.nowstage=? OR projectparams.nowstage=?) AND members.user_id=?",3,4,current_user.id).all

    @question = Questionforidea.where(:show=>true)
  end

	def investlist	#我的提案,給提案者看目前贊助列表
		@list = Investment.includes(:pro).order("updated_at ASC").where("pro_id=?",params[:id])
	end
