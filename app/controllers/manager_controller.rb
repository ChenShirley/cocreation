...
#      end
  ...
  def index #管理員後台首頁
    ...
    @alldelete = Projectparam.includes(:presub).where("nowstage IS NOT NULL").all	#管理員有權把草案刪掉&修改參數
		#審核匯款
    @check = Investment.includes(:pro).includes(:user).order("pro_id ASC").order("updated_at ASC").where("(confirm='f' OR confirm IS NULL) AND remit='t'")
    @unremit = Investment.includes(:pro).includes(:user).order("pro_id ASC").order("updated_at ASC").where("remit IS NULL AND confirm IS NULL")
    @already = Investment.includes(:pro).includes(:user).order("pro_id ASC").order("updated_at ASC").where("confirm='t'")
    @prolist = Presub.includes(:pro).where("pros.teacheron=?",true)
  end

  def deleteinvest
		@invest = Investment.find(params[:id])
		if @invest.remit.nil? #未送出匯款完成通知
			@invest.destroy
  ...
  def update
    ...
    @feedback = Feedback.where(:id=>@check.feedback_id)
      ...
      Usernotice.sendemail(@check.user.email,'co-creation 專案通知','您的匯款已完成，在募資截止後，若專案募款達到預期金額，提案者將依您所贊助的金額履行承諾的項目。若專案募資未達預期金額，Cocreation平台會寄送信件詢問您的退款帳戶，並將您的贊助金額全數退還，謝謝您這次的贊助。邀請您持續關注此專案，如有專案相關問題，請隨時與專案發起人'+@check.pro.presub.user.name+'('+@check.pro.presub.user.email+')保持聯繫。您可利用此網址回到Cocreation平台查看您的贊助資訊：http://cocreation.ctm.nthu.edu.tw/startaproject/newindex').deliver
    	redirect_to manager_index_path
		end
  end
